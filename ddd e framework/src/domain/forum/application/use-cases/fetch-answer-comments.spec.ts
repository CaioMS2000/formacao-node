...
describe("Fetch answer comments", () => {
    beforeEach(() => {
        ...
        useCase = new FetchAnswerCommentsUseCase(inMemoryAnswerCommentRepository);
    });

    test("should be able to fetch answer comments", async () => {
        await inMemoryAnswerCommentRepository.create(makeAnswerComment({answerId: new UniqueId('1')}));
        await inMemoryAnswerCommentRepository.create(makeAnswerComment({answerId: new UniqueId('1')}));
        await inMemoryAnswerCommentRepository.create(makeAnswerComment({answerId: new UniqueId('1')}));
