...
describe("Edit question", () => {
    ...
    test("should be able to edit a question", async () => {
        ...
        await useCase.execute({userId: "author-id", title: "título de teste", content: "conteudo de teste", questionId: "any-id"});
    
        expect(inMemoryQuestionsRepository.questions[0]).toMatchObject({title: "título de teste", content: "conteudo de teste"})
    });
    ...
    test("should not be able to edit a question from another user", async () => {
        ...
        const result = await useCase.execute({userId: "author-id-1", title: "título de teste", content: "conteudo de teste", questionId: "any-id"})
