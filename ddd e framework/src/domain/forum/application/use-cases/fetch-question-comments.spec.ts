...
describe("Fetch question comments", () => {
    beforeEach(() => {
        ...
        useCase = new FetchQuestionCommentsUseCase(inMemoryQuestionCommentRepository);
    });

    test("should be able to fetch question comments", async () => {
        await inMemoryQuestionCommentRepository.create(makeQuestionComment({questionId: new UniqueId('1')}));
        await inMemoryQuestionCommentRepository.create(makeQuestionComment({questionId: new UniqueId('1')}));
        await inMemoryQuestionCommentRepository.create(makeQuestionComment({questionId: new UniqueId('1')}));
