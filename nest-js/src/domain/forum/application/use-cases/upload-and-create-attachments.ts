...
export class UploadAndCreateAttachmentUseCase{
    constructor(private attachmentsRepository: AttachmentsRepository, private fileUploader: Uploader){}
    ...
    async execute({body, fileName, fileType}: UploadAndCreateAttachmentUseCaseRequest): Promise<UploadAndCreateAttachmentUseCaseResponse> {
        if(!/^(image\/(jpeg|jpg|png))$|^application\/pdf$/.test(fileType)) return left(new InvalidAttachmentTypeError());

        const {url} = await this.fileUploader.upload({fileName, fileType, body})
